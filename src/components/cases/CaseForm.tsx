...
import { mockUsers } from "@/data/mockData"; // For lawyer list
...
export function CaseForm({ initialData, onSave, onDelete }: CaseFormProps) {
  ...
  const lawyers = mockUsers.filter((user) => user.role === UserRole.LAWYER);
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ej: 202300101" {...field} readOnly={!isAdmin && !!initialData} />
                    </FormControl>
                    {!isAdmin && !!initialData && <FormDescription>Solo administradores pueden editar el NUREJ.</FormDescription>}
                    <FormMessage />
                  </FormItem>
                )}
              />
